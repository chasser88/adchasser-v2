...
// ── Get respondent_id for current user ────────────────────────────
async function getCurrentRespondentId() {
  try {
    ...
    // Fallback by email and fix stale user_id
    if (user.email) {
      const { data: byEmail } = await supabase
        ...
        .maybeSingle()
      if (byEmail) {
        await supabase.from('respondents').update({ user_id: user.id }).eq('id', byEmail.id)
        return byEmail.id
      }
    }
    return null
...
export async function createResponseRecord(campaignId) {
  const respondentId = await getCurrentRespondentId()
...
export async function submitSurveyResponse({ campaignId, responseId, track, answers, completionPct }) {
  ...
  for (let i = 0; i < answerRows.length; i += batchSize) {
    const { error: ansErr } = await supabase.from('survey_answers').insert(answerRows.slice(i, i + batchSize))
