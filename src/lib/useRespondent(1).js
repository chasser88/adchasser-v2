...
// ── Get available surveys for respondent — server-side filtered ──
export async function getAvailableSurveys(respondent) {
  if (!respondent?.id) return []

  // Use server-side function — all filtering done in PostgreSQL
  const { data, error } = await supabase.rpc('get_available_surveys', {
    p_respondent_id: respondent.id
  })

  if (error) {
    console.error('getAvailableSurveys error:', error)
    return []
  }

  if (!data?.length) return []

  // Apply demographic matching client-side (lightweight)
  return data.filter(c => matchesDemographic(c, respondent))
}
...
function matchesDemographic(campaign, respondent) {
  ...
  // If respondent has no location set — show all
