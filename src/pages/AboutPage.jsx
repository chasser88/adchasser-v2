...
export default function AboutPage({ user }) {
  ...
  const story   = block('text_section')
  const VALUES  = block('cards')?.items       ?? DEFAULT_VALUES
  const contact = block('contact')

  const heroSub       = hero?.subheadline       ?? 'AdChasser was born out of a simple frustration: the tools available to measure brand campaigns were either too expensive, too slow, or too shallow to drive real decisions.'
  const storyHeadline = story?.headline         ?? 'The problem we set out to solve'
  const paragraphs    = story?.paragraphs       ?? DEFAULT_STORY
  const valuesHeadline = block('cards')?.headline ?? 'What we believe'
  const contactHeadline = contact?.headline     ?? 'Get in touch'
  const contactSubtext  = contact?.subtext      ?? 'For enterprise enquiries, partnerships, or general questions — we\'d love to hear from you.'
  const contactEmail    = contact?.email        ?? 'hello@adchasser.com'
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: F.sans, minHeight: '100vh' }}>
      ...
      <section style={{ padding: 'clamp(48px,8vw,96px) 5%', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 'clamp(14px,2vw,17px)', color: C.muted, lineHeight: 1.8, maxWidth: '580px', margin: '0 auto' }}>{heroSub}</p>
      </section>

      {/* Story */}
      <section style={{ padding: 'clamp(40px,6vw,80px) 5%', background: C.surface }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', fontFamily: F.display, fontWeight: 700, marginBottom: '24px' }}>{storyHeadline}</h2>
      ...
      <section style={{ padding: 'clamp(40px,6vw,80px) 5%' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', fontFamily: F.display, fontWeight: 700, marginBottom: '40px', textAlign: 'center' }}>{valuesHeadline}</h2>
      ...
      <section id="contact" style={{ padding: 'clamp(40px,6vw,80px) 5%', background: C.surface }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', fontFamily: F.display, fontWeight: 700, marginBottom: '14px' }}>{contactHeadline}</h2>
          <p style={{ fontSize: '15px', color: C.muted, lineHeight: 1.75, marginBottom: '32px' }}>{contactSubtext}</p>
          <a href={`mailto:${contactEmail}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '13px 28px', background: `linear-gradient(135deg,${C.gold},${C.goldLight})`, border: 'none', borderRadius: '10px', color: C.bg, fontSize: '14px', fontWeight: 700, fontFamily: F.sans, textDecoration: 'none' }}>
            📧 {contactEmail}
          </a>
        </div>
      </section>
