...
const DEFAULT_PLANS = [
  { name: 'Starter', price: 'Free', sub: 'Forever free', color: C.muted, desc: 'Perfect for trying AdChasser on your first campaign.', features: ['1 active campaign', 'Up to 200 responses', 'Full 38-question survey', 'Live dashboard', 'PDF export', 'Email support'], cta: 'Get Started Free', cta_url: '/auth?signup=true', popular: false },
  { name: 'Professional', price: '$149', sub: 'per month', color: C.gold, desc: 'For agencies and brand teams running multiple campaigns.', features: ['Unlimited campaigns', 'Unlimited responses', 'Full 38-question survey', 'Live dashboard', 'PDF export', 'Priority support', 'Custom branding on survey', 'Team access (up to 5 seats)'], cta: 'Start Free Trial', cta_url: '/auth?signup=true', popular: true },
  { name: 'Enterprise', price: 'Custom', sub: 'contact us', color: C.blue, desc: 'For large organisations with bespoke research needs.', features: ['Everything in Professional', 'Unlimited team seats', 'White-label surveys', 'Custom question modules', 'Dedicated account manager', 'SLA guarantee', 'Custom integrations', 'On-site training'], cta: 'Contact Us', cta_url: '/about#contact', popular: false },
]
...
const planColor = (plan) => {
  ...
  if (plan.popular) return C.gold
  if (plan.name === 'Enterprise') return C.blue
  return C.muted
}

export default function PricingPage({ user }) {
  const navigate = useNavigate()
  const { block } = useCMSPage('pricing')

  const PLANS = block('pricing')?.plans ?? DEFAULT_PLANS
  const FAQS  = block('faq')?.items     ?? DEFAULT_FAQS
  const hero  = block('hero')
  const faqHeadline = block('faq')?.headline ?? 'Frequently Asked Questions'
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: F.sans, minHeight: '100vh' }}>
      ...
      <section style={{ padding: 'clamp(48px,8vw,96px) 5%', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 'clamp(28px,5vw,52px)', fontFamily: F.display, fontWeight: 700, marginBottom: '16px' }}>
          {hero?.headline ?? 'Simple, transparent pricing'}
        </h1>
        <p style={{ fontSize: 'clamp(14px,2vw,17px)', color: C.muted, maxWidth: '500px', margin: '0 auto' }}>
          {hero?.subheadline ?? 'Start free. Scale when you\'re ready. No hidden fees.'}
        </p>
      </section>
            ...
            return (
              <div key={plan.name} style={{ padding: '28px', background: plan.popular ? `linear-gradient(135deg,${C.gold}0A,${C.card})` : C.card, border: `1px solid ${plan.popular ? C.gold+'50' : C.border}`, borderRadius: '20px', position: 'relative' }}>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <p style={{ fontSize: '11px', fontWeight: 600, color, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '6px', fontFamily: F.sans }}>{plan.name}</p>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '6px' }}>
                    <span style={{ fontSize: 'clamp(28px,4vw,36px)', fontWeight: 700, fontFamily: F.display, color }}>{plan.price}</span>
                ...
                <button onClick={() => navigate(plan.cta_url ?? '/auth?signup=true')} style={{ width: '100%', padding: '12px', background: plan.popular ? `linear-gradient(135deg,${C.gold},${C.goldLight})` : 'transparent', border: plan.popular ? 'none' : `1px solid ${C.border}`, borderRadius: '10px', color: plan.popular ? C.bg : C.text, fontSize: '14px', fontWeight: 700, fontFamily: F.sans, cursor: 'pointer', marginBottom: '20px' }}>
                  {plan.cta}
                </button>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '9px' }}>
                  {(plan.features ?? []).map(f => (
                    <li key={f} style={{ display: 'flex', alignItems: 'center', gap: '9px', fontSize: '13px', color: C.muted, fontFamily: F.sans }}>
                      <div style={{ width: '16px', height: '16px', borderRadius: '50%', background: color+'18', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '9px', color }}> ✓</div>
                      {f}
                    </li>
                  ))}
                </ul>
      ...
      <section style={{ padding: 'clamp(40px,6vw,80px) 5%', background: C.surface }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', fontFamily: F.display, fontWeight: 700, textAlign: 'center', marginBottom: '40px' }}>{faqHeadline}</h2>
